...
class _Command(object):
    """This class is used internally by the @command decorator
    to store command information on a method.
    """

    def __init__(self, method_name, name, kwargs, arguments):
        if not isinstance(method_name, str):
            raise TypeError(
                "Parameter 'method_name' is invalid. "
                "Expected a str instead of {got_type}.".format(
                    got_type=type(method_name).__name__))
        ...
        if not isinstance(kwargs, dict):
            raise TypeError(
                "Parameter 'kwargs' is invalid. "
                "Expected a dict instead of {got_type}.".format(
                    got_type=type(kwargs).__name__))
        if not isinstance(arguments, list):
            raise TypeError(
                "Parameter 'arguments' is invalid. "
                "Expected a list instead of {got_type}.".format(
    ...
    def __call__(self, target, *args, **kwargs):
        method = getattr(target, self.method_name, None)
        if not method:
            raise AttributeError(
                "Can't call command '{name}'. "
                "Method {method_name} does not exist on {cls}.".format(
                    name=self.name,
        ...
        if not callable(method):
            raise TypeError(
                "Can't call command '{name}'. "
                "Attribute {method_name} on {cls} is not a method.".format(
                    name=self.name,
                    method_name=self.method_name,
                    cls=type(target).__name__))
...
def command(*args, **kwargs):
    ...
    def decorator(func):
        ...
        if not func or not callable(func):
            raise TypeError(
                "Decorator @{command} should be added to a method "
        ...
        func.__command__ = _Command(
            func.__name__, command_name, kwargs, arguments)
...
def getcommand(cls, name):
    ...
    if not isinstance(cls, type):
        raise TypeError(
            "Parameter 'cls' should be a class "
            "instead of {}.".format(
                type(cls).__name__))
    if not isinstance(name, str):
        raise TypeError(
            "Parameter 'name' should be a str "
            "instead of {}.".format(
                type(name).__name__))
