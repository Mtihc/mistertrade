...
class _Command(object):
  ...
  def __init__(self, method_name, name, kwargs, arguments):
    if not isinstance(method_name, str):
      raise TypeError("Parameter 'method_name' is invalid. Expected a str instead of {got_type}.".format(got_type=type(method_name).__name__))
  ...
  def __call__(self, target, *args, **kwargs):
    ...
    if not method:
      raise AttributeError("Can't call command '{name}'. Method {method_name} does not exist on {cls}.".format(name=self.name, method_name=self.method_name, cls=type(target).__name__))
    if not callable(method):
      raise TypeError("Can't call command '{name}'. Attribute {method_name} on {cls} is not a method.".format(name=self.name, method_name=self.method_name, cls=type(target).__name__))
...
def command(*args, **kwargs):
  ...
  def decorator(func):    
    ...
    func.__command__ = _Command(func.__name__, command_name, kwargs, arguments)
