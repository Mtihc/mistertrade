...
def exchange_name(name):
  ...
  if not name:
    raise ValueError("Parameter name can't be '{}'.".format(name))
  if not isinstance(name, str):
    raise TypeError("Parameter name should be a string instead of {}.".format(type(name).__name__))
  if not _exchange_name_pattern.match(name):
    raise ValueError("Parameter name can't be '{}'. It should match the following pattern: {}".format(name, _exchange_name_pattern.pattern))
  ...
  def decorator(klass):
    ...
    if not isinstance(klass, type):
      raise TypeError("The @{decorator} decorator should be applied to a class instead of {got_type}.".format(decorator=exchange_name.__name__, got_type=type(klass).__name__))
...
class ExchangeAPI(metaclass=abc.ABCMeta):
  ...
  def price_with_fee(self, buy_or_sell, price, base_coin, precision=8):
    ...
    if buy_or_sell not in ['buy', 'sell']:
      raise ValueError("Parameter buy_or_sell must be 'buy' or 'sell' instead of '{}'.".format(buy_or_sell))
  ...
  def validate_minimum_trade_size(self, market, quantity, rate):
    ...
    if quantity < minimum_trade_size:
      raise MinimumTradeSizeError(market_coin=market_coin, quantity=quantity, minimum_trade_size=minimum_trade_size)
...
class ExchangeCLI(metaclass=abc.ABCMeta):
